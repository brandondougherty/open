...
extension String {
    ...
    */
    var escaped: String {
        return CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault,self,"[].",":/?&=;+!@#$()',*",CFStringConvertNSStringEncodingToEncoding(NSUTF8StringEncoding)) as AnyObject as! String
    }
}
    ...
    */
    func createRequest(url: NSURL, method: HTTPMethod, parameters: Dictionary<String,AnyObject>?) -> (request: NSURLRequest, error: NSError?) {
        ...
        if isMulti {
            ...
            if parameters != nil {
                request.HTTPBodyStream = NSInputStream(data : dataFromParameters(parameters!,boundary: boundary))
            }
